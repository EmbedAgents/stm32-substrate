...
class TestTargetControl:
    def test_halt(self, ctx: SubstrateContext) -> None:
        gdb = MagicMock()
        session = _make_session(ctx, gdb)
    ...
    def test_reset_halt_after(self, ctx: SubstrateContext) -> None:
        ...
        gdb.send_console.assert_called_with("monitor reset halt", timeout_s=10.0)
        assert session.target_halted is True
...
class TestReadMemory:
    def test_canonical_invocation(self, ctx: SubstrateContext) -> None:
        ...
        gdb.send_mi.assert_called_with(
            "-data-read-memory-bytes 0x20000000 16", timeout_s=10.0
        )
